...
class noise_test:
    ...
    def run(self):
        ...
        if self._noise_test_result == None:
            ...
            self._log("correct_num:{}, error_num:{}, correate_rate:{}".format(self._correct_num, self._error_num,
                                                                              self._correct_num / (self._correct_num + self._error_num)))
    ...
    def _response_process(self, case_batch):
        for case in case_batch:
            ...
            self._log("\ncorrect answer is {}\n".format(label))
            raw_answer = messages[-1][-1]["content"]
    ...
    def _query_process(self):
        ...
        for index, case_batch in enumerate(case_list):
            n_case_batch = self._method6_contrastive_highlight_noise(case_batch)
            ...
            for case in n_case_batch:
                print("in-context:{}".format(case['in-context']))
                print("question:{}".format(case['question']))
                print("answer:{}".format(case["messages"][-1][-1]['content']))
                print('label:{}'.format(case['label']))
        ...
        for case in case_batch:
            ...
            for shot in in_context:
                ...
                contrastive_question = "The following are two examples for the same kind of tasks: " \
                                       ...
                                       "which must be similar to the excellent one."
    ...
    def rephrase_SC(self, rephrase_list):
        answers = []
        for shot in rephrase_list:
            raw_answer = shot[1]
            answer = self._dataset_processor.match_answer(raw_answer)
...
def COT_SC_correct_rate(answers_list):
    ...
    print("SC_correct_num:{}, vaild_num:{}, SC_correct_rate:{}".format(SC_right_count, valid_count,
